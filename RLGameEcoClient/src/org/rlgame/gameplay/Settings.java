...
import gr.eap.RLGameEcoClient.extra.FileManagement;
...
//	public static final int PLIES = 3;
	...
	public static final int RL2_PLAYER = 5;
	public static final int RL3_PLAYER = 6;
	public static final int MARL1_PLAYER = 7;
	...
	public static double winnerReward	= 100;
...
//	public static int blackNeuralHiddenSize = (DIMBOARD * DIMBOARD - 2 * DIMBASE * DIMBASE + 5);	
	...
	public static String playerModeSet(int turn, int val) {
		...
		if (turn == Settings.WHITE_PLAYER) {
			if (val > 0 && val <= 7) {
				...
				if (val == Settings.RL2_PLAYER) {
					playerMode = "RL2";
				}
				if (val == Settings.RL3_PLAYER) {
					playerMode = "RL3";
				}
				if (val == Settings.MARL1_PLAYER) {
					playerMode = "MARL1";
				}				
			}
		...
		/* development log */ FileManagement.appendToFile("/tmp/development.log", Client.me.getUserName()+": Settings - playerModeSet: "+ playerMode);
		return playerMode;
	}
	...
	public static String getPlayerMode(int turn) {
		String playerMode = "";
		if (turn == Settings.WHITE_PLAYER) {
			if (Settings.PLAYER_W_MODE == Settings.RL_PLAYER) {
				playerMode = "RL";
			}
			...
			if (Settings.PLAYER_W_MODE == Settings.RL2_PLAYER) {
				playerMode = "RL2";
			}
		} else if (turn == Settings.BLACK_PLAYER) {
			...
			if (Settings.PLAYER_B_MODE == Settings.RL2_PLAYER) {
				playerMode = "RL2";
			}
		}
