...
//		}
	...
	public void pawnsToBinaryArray() {
	...
	public double getRewardNew(int turn) {		
		// input value turn stands for the color of the the client who checks for reward value
		//int stateId= this.getTurn();	// the color in GameState
		...
		if(finalState) {
			if(whiteLeft == 0)							// white player has no pawns
				if(turn ==Settings.WHITE_PLAYER)
					return -Settings.winnerReward;		// white player looses
				else
					return Settings.winnerReward;		// black player wins
			else if(blackLeft == 0)						// black player has no pawns
				if(turn ==Settings.WHITE_PLAYER)
					return Settings.winnerReward;		// white player wins
				else
					return -Settings.winnerReward;		// black player looses
			else										// player has reached enemy's base
				if (this.getTurn() == Client.machine.getId())
					return Settings.winnerReward;		// player wins
				else
					return -Settings.winnerReward;		// player looses
					
			//		
			//if(turn == Settings.WHITE_PLAYER) {				// white client
			//	if(whiteLeft == 0)							// ... white has no pawns to play
			//		return -Settings.winnerReward;
			//	else if (blackLeft == 0)					// ... black has no pawns to play
			//		return Settings.winnerReward;
			//	else										// ... both players have active pawns
			//		if(stateId == turn)						// ...... final state on white player
			//			return Settings.winnerReward;
			//		else									// ...... final state on black player
			//			return -Settings.winnerReward;
			//} else {										// black client
			//	if(whiteLeft == 0)							// ... white has no pawns to play
			//		return Settings.winnerReward;
	...
	public double getReward(int turn) {			
		...
		return getReward(turn, whiteLeft, blackLeft, finalState);												// no final state
	}
	...
	public void setPawns(Pawn[] white, Pawn[] black) {
		whitePawns = white.clone();
		blackPawns = black.clone();
	}
