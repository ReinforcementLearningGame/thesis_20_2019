...
/* ---------------- Constructor ---------------- */	  
	public Agent(byte boardSize, byte baseSize, byte numberOfPawns){
		...
		this.theta_name = "RL2_theta"+"_"+boardSize+"_"+baseSize+"_"+numberOfPawns;
	...
	public void applySelectedMoveReward (int[][] pawnsCurrent, AgentAction moveCurrent, double reward, 
		...
		double ef=0.0;						//tr(e)*f
		for (int i=0; i<8*numberOfPawns;i++)
			ef += elig[i]*featCurr[i];
		for (int i=0; i<8*numberOfPawns;i++)
			elig[i] = gamma*lambda*elig[i] + (1.0 - alpha*gamma*lambda*ef)*featCurr[i];
		for (int i=0; i<8*numberOfPawns;i++)
			theta[i] += alpha*(d+Qc-Qo)*elig[i] - alpha*(Qc-Qo)*featCurr[i];
		Qo = Qn;
	}
	//
...
/* ---------------- Approximation Functions ---------------- */
	...
	//
	private double func05(int[] pawn, int[] move) {					// feature A*lnd
		return func01(pawn)*func02(pawn, move);
	}
	//
	private double func06(int[] pawn, int[] move) {					// feature Z*lnd
		return func01(pawn)*func03(pawn, move);
	}
	//
	private double func07(int[] pawn, int[] move) {					// feature A*Z*lnd
		return func01(pawn)*func04(pawn, move);
	}
	//
